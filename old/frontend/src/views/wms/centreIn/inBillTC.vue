        ...
        <el-table-column label="操作" width="250" align="center">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              v-show="scope.row.billStatus==1 && scope.row.createType==2">打印
            </el-button>
            <!--       <el-button
                    size="mini"
                    type="primary"
                    @click="handlePrint(scope.$index, scope.row)"
                    v-show="scope.row.billStatus==1 && scope.row.createType==4">补打
                  </el-button>-->
            <el-button
              size="mini"
              type="primary"
...
export default {
  ...
  methods: {
    ...
    handleViewEdit(index, row) {
      if (row.createType == 2) {
        this.$router.push({
          path: '/wms/createInBillTC',
          query: {billCode: row.billCode}
        })
      } else if (row.createType == 4) {
        this.$router.push({
          path: '/wms/scanInBillTC',
