    ...
    <div class="table-container">
      <el-table ref="inBillItemTable"
        ...
        <el-table-column label="状态" align="center">
          <template slot-scope="scope">{{ statusData[(scope.row.status)] }}</template>
        </el-table-column>
        <!--        <el-table-column label="操作" width="140" align="center">
                  <template slot-scope="scope">
                    <el-button
                      size="mini"
                      type="primary"
                      @click="handleSupplement(scope.$index, scope.row)">补打
                    </el-button>
                  </template>
                </el-table-column>-->
      </el-table>
...
<script>
import {fetchList} from '@/api/inBillItem'
  ...
  methods: {
    ...
    printCode() {
      setTimeout(() => {
        ...
        this.$nextTick(() => {
          ...
          for (let i = 0; i < that.inBillItem.length; i++) {
            doc.write(`
                <div class="long">
                         <div class="qrcode">${arrDom[i].innerHTML}</div>
                         <div class="codeValue">${that.inBillItem[i].codeValue}</div>
                </div>
    ...
    getList() {
      ...
      fetchList(this.listQuery).then(response => {
        ...
        this.inBillItem = response.data.list;
