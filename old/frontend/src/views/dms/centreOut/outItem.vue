  ...
  <div class="app-container">
    ...
    <div class="table-container">
      <el-table ref="outItemTable"
        ...
        <el-table-column label="出库单编号" min-width="160%" align="center">
...
<script>
import {fetchList} from '@/api/outItem'
...
export default {
  ...
  methods: {
    ...
    printCode() {
      setTimeout(() => {
        ...
        this.$nextTick(() => {
          ...
          for (let i = 0; i < that.outItem.length; i++) {
            doc.write(`

                <div class="long">
                         <div class="qrcode">${arrDom[i].innerHTML}</div>
                         <div class="codeValue">${that.outItem[i].codeValue}</div>
                </div>
    ...
    getList() {
      ...
      this.listQuery.keyword = this.$route.query.outDetailId;
      fetchList(this.listQuery).then(response => {
        this.listLoading = false;
        this.outItem = response.data.list;
