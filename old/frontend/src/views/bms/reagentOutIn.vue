  ...
  <div class="app-container">
    ...
    <div class="table-container">
      ...
      <el-table ref="reagentTable"
        ...
        <el-table-column v-if="colData[0].istrue" prop="inQuantity" label="入库数量" align="center">
          <template slot-scope="scope">{{ scope.row.inQuantity }}</template>
        </el-table-column>
        <el-table-column v-if="colData[1].istrue" prop="inTotal" label="入库金额(元)" align="center">
          <template slot-scope="scope">{{ scope.row.inTotal }}</template>
        </el-table-column>
        <el-table-column v-if="colData[2].istrue" label="退货数量" prop="refundQuantity" align="center">
          <template slot-scope="scope">{{ scope.row.refundQuantity }}</template>
        </el-table-column>
        <el-table-column v-if="colData[3].istrue" label="退货金额(元)" prop="refundTotal" align="center">
          <template slot-scope="scope">{{ scope.row.refundTotal }}</template>
        </el-table-column>
        <el-table-column v-if="colData[4].istrue" label="出库数量" prop="outQuantity" align="center">
          <template slot-scope="scope">{{ scope.row.outQuantity }}</template>
        </el-table-column>
        <el-table-column v-if="colData[5].istrue" label="出库金额(元)" prop="outTotal" align="center">
          <template slot-scope="scope">{{ scope.row.outTotal }}</template>
        </el-table-column>
        <el-table-column v-if="colData[6].istrue" label="净入库数量" prop="cleanQuantity" align="center">
          <template slot-scope="scope">{{ scope.row.cleanQuantity }}</template>
        </el-table-column>
        <el-table-column v-if="colData[7].istrue" label="净入库金额(元)" prop="cleanTotal" align="center">
          <template slot-scope="scope">{{ scope.row.cleanTotal }}</template>
        </el-table-column>
      </el-table>
...
import {reagentOutIn} from '@/api/stockDetail'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.formatList = [];
      reagentOutIn(this.listQuery).then(response => {
        ...
        this.list.forEach(item => {
          const obj = {
            ...
            cleanQuantity: item.cleanQuantity,
            cleanTotal: item.cleanTotal,
          }
